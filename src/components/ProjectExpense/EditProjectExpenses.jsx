...
const EditProjectExpenses = () => {
  ...
  useEffect(() => {
    const fetchProjectData = async () => {
      try {
        const response = await fetch(`http://localhost:3000/projectexpenses/edit/${id}`);
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
        const data = await response.json();
        setProjects(data.projects);
        setValues(data.expense)
...
export default EditProjectExpenses;
